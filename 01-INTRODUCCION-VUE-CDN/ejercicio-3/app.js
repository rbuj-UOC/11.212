...
createApp({
  setup() {
    const showOnlyInTheaters = ref(false);

    const filteredMovies = computed(() => {
      if (!showOnlyInTheaters.value) {
        return movies;
      }
    ...
    const filterButtonText = computed(() =>
      showOnlyInTheaters.value ? "Mostrar todas" : "En cines"
    );

    const toggleInTheatersFilter = () => {
      showOnlyInTheaters.value = !showOnlyInTheaters.value;
    };
